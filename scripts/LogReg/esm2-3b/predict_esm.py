...
df_summary = pd.DataFrame(columns=['PDBcode', 'CLS', 'Samples', 'F1 Score', 'MCC_score'])
...
for protein in tqdm(proteins):
    ...
    try:
        if length < 1023:
            ...
            # Structure--------------------------------------------------------------------
            for contact in os.listdir('../15051/contacts/'): 
                if contact[:8] == proteinid:
                    structure = np.load('../15051/contacts/'+contact)['b']
            ...
            df.loc[len(df)] = [rnaid, mode, N, round(f1_score,2), round(mcc_score,2)]
...
df_summary.loc[len(df)] = ['average', mode, N, round(total_f1/count,2), round(total_mcc/count,2)]

df.to_csv(f'./wpreds/analytical_evaluation_3D_ESM3B.csv', index=False)
df_summary.to_csv(f'./wpreds/evaluation_3D_ESM3B_rf.csv', index=False)
