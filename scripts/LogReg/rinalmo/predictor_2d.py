...
for mode in ["rf"]:#["logreg", "mlp", "tr", "rf", "xgb"]:
    for N in ["20"]:#["20", "500", "2500", "5000", "8000", "25715"]:

        if mode == 'logreg':
            from sklearn.linear_model import LogisticRegression
            print('Running logistic regression. (cpu)')

            chosen_model = joblib.load(f"./experiments/logreg/models_2d/{lm}_logreg_{N}.joblib")
            ...
            chosen_model = joblib.load(f'./experiments/logreg/models_2d/{lm}_mlp_{N}.joblib')
            ...
            chosen_model = joblib.load(f'./experiments/logreg/models_2d/{lm}_tr_{N}.joblib')
            outname = f'{lm}_tr_{N}'
            ...
            chosen_model = joblib.load(f'./experiments/logreg/models_2d/{lm}_rf_{N}.joblib')
            outname = f'{lm}_rf_{N}'
            model = 'Random Forest'
            ...
            chosen_model = joblib.load(f'./experiments/logreg/models_2d/{lm}_xgb_{N}.joblib')
        ...
        for rna in tqdm(rnas):
            ...
            try:
                ...
                structure = dot_bracket_to_matrix(s_structure)
                ...
                np.savez(f'../../benchmark/lms/{lm}2d/{rnaid}.npz', b=y_pred.reshape(length,length))
                ...
                #save = './experiments/logreg/visuals/' + f'{rnaid}_2D_{lm}_{model}.pdf'
...
#df.to_csv(f'./experiments/logreg/analytical_evaluation_2D_{lm}.csv', index=False)
df.to_csv(f'./experiments/logreg/indipedents_2D_{lm}_{N}.csv', index=False)
