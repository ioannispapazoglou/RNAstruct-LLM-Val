...
class CustomAttentionClassifier(nn.Module):
    def __init__(self, length):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(72, 64, kernel_size=3, padding=1), # 72 is 6 * 12, reshaped attention maps
            nn.ReLU(),
            nn.MaxPool2d(2),
...
def remove_lm_padding(padded_attention, original_length):
...
pretrained = set_learned_params(model,'../otherlms/RNABERT/bert_mul_2.pth')
...
for epoch in range(num_epochs):
    ...
    for rna in tqdm(trainingrnas): # Every batch constitutes a molecule 
        ...
        if length <= 438:
            ...
            attention_weights = torch.tensor(remove_lm_padding(padded_attention, length))

            attention_maps = pad_attention(attention_weights, 1024)
            attention_maps = attention_maps.view(1, 72, 1024, 1024).float().to(device) #(1, LxH, maxl, maxl)
