...
class CustomAttentionClassifier(nn.Module):
    def __init__(self, length):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(240, 64, kernel_size=3, padding=1), # 240 is 12 * 20, reshaped attention maps
            nn.ReLU(),
            nn.MaxPool2d(2),
...
for epoch in range(num_epochs):
    ...
    for rna in tqdm(trainingrnas): # Every batch constitutes a molecule 
        ...
        if length <= 700:
            ...
            attention_maps = pad_attention(attention_weights[:, :, 1:-1, 1:-1], 1024)
            attention_maps = attention_maps.view(1, 240, 1024, 1024).float().to(device) #(1, LxH, maxl, maxl)
