...
#cutoff = '8'
with open('../data/2dset<512.json') as f:
    rnas = json.load(f)
...
for rna in tqdm(rnas):
    ...
    try:
        if length >= 20:
            ...
            #structure_3mer = np.load(f'../data/contacts{cutoff}_nodiag_3mer/pdb'+rnaid+'_contacts.npz')['b']
...
np.savez_compressed(f'dnabert_2D_probability-{th}.npz', p=probability)
...
def heatdouble(heat,th):
    ...
    #plt.show()
    plt.savefig(f'./dna_2D-{th}.pdf', format='pdf')
