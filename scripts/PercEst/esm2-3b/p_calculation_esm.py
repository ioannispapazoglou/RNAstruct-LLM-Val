...
def calculate_p(contactmap, attentionmap, th):
    ...
    #attentionmap_mask = attentionmap < 0.3  # This for better plots ! ! ! ! ! ! ! ERASE
...
for protein in tqdm(proteins):
    ...
    try:
        ...
        for contact in os.listdir('./pdb_nodiag/'):
            if contact[:8] == proteinid:
                structure = np.load('./pdb_nodiag/'+contact)['b']
...
np.savez_compressed(f'15k_esm2-p-{mode}-{th}-nodiag.npz', p=probability)
...
def heatdouble(heat,th):
    ...
    plt.savefig(f'15k_esm2-{mode}-{th}-nodiag.pdf', format='pdf')
